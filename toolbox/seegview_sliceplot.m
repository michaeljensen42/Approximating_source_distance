...
function [x_slice, y_slice, z_slice] = seegview_sliceplot(locs, V, x, y, z, slicethickness, scale, side)
...
%% z - plotting (axial slices)
    ...
    for k=1:length(z_slice.values)    
        ...
        if side == 'R'
        curr_image=z_slice.images((x>-5),((-95<y)&(y<95)),k);
        surf(x(x>-5),y((-95<y)&(y<95)),.001*permute(curr_image, [1 2 3]).'-100,'edgecolor','none'), view(0,90)
        elseif side == 'L'
        curr_image=z_slice.images((x<5),((-95<y)&(y<95)),k);       
        surf(x(x<5),y((-95<y)&(y<95)),.001*permute(curr_image, [1 2 3]).'-100,'edgecolor','none'), view(0,90)
        elseif side == 'B'
        surf(x,y,.001*squeeze(z_slice.images(:,:,k)).'-100,'edgecolor','none'), view(0,90)
        curr_image=z_slice.images(:,((-95<y)&(y<95)),k);
...
%          surf(x,y,.001*squeeze(z_slice.images(:,:,k)).'-100,'edgecolor','none'), view(0,90)
        ...
        text((max(x)/2)+10, (max(y)/2)+10, -100, sprintf('z=%.1f', z_slice.values(k)), 'color', 'w');
    end
    z_slice.ha=haz;
    z_slice.perm = [1 2 3];
...
%% y - plotting (coronal slices)
    ...
    for k=1:length(y_slice.values)    
         axes(hay(k))
         if side == 'R'
         curr_image=y_slice.images((x>-5),k,(z>-40));
         surf(x(x>-5),z(z>-40),.001*permute(curr_image,[1 3 2]).'-100,'edgecolor','none'), view(0,90)
         elseif side == 'L'
         curr_image=y_slice.images((x<5),k,(z>-40));
         surf(x(x<5),z(z>-40),.001*permute(curr_image,[1 3 2]).'-100,'edgecolor','none'), view(0,90)
         elseif side == 'B'
%          curr_image=y_slice.images(:,k,(z>-40));
%          surf(x,z(z>-40),.001*permute(curr_image,[1 3 2]).'-100,'edgecolor','none'), view(0,90)
           curr_image=y_slice.images(:,k,:);
        ...
        axis equal, axis off, axis tight
        text((max(x)/2)+10, (max(z)/2)+10, -100, sprintf('y=%.1f', y_slice.values(k)),'color', 'w')
...
%% x - plotting (sagittal slices)
    ...
    for k=1:length(x_slice.values)    
        ...
        axis equal, axis off, axis tight
        text((max(y)/2)+10, (max(z)/2)+10, -100, sprintf('x=%.1f', x_slice.values(k)),'color', 'w')
    end
    x_slice.ha=hax;
    x_slice.perm=[2 3 1];    
