...
%% Calculate sets of normalized single stimulation cross-projection magnitudes
    ...
    for k=proj_tpts % parse through time and perform projections for different data lengths
        ...
        S_all(:,length(m))=S; % store projection weights 
                ...
                [mp,tt]=max(m); % tt is the sample corresponding to response duration
            
            thresh_pct = 0.99;

            bthresh=find((m(1:(end-1))>(thresh_pct*mp)).*(m(2:end)<=(thresh_pct*mp))); % using the % threshold
            bthresh(bthresh>tt)=[];
            if isempty(bthresh), bthresh = 0;, end
            bthresh = bthresh(1);

            relthresh = mp - (mp - m(end))/10;

            brelthresh = (m(1:(end-1))<(relthresh)); %.*(m(2:end)<=(relthresh))); % using the % threshold
            brelthresh(1:tt) = 0;
            brelthresh = find(brelthresh == true);
            if isempty(brelthresh)
                brelthresh = length(m);
            else
                brelthresh = brelthresh(1); % earliest time point m < relthresh
            end

            b = max(bthresh, brelthresh);
            ...
            if isempty(b), hb=length(m)-1; % in case it goes to end,
                else, lb=b(end);
            end

           mhb = floor(tt + (0.2*(hb-tt)));  % moderate upper bound when upper bound takes us to the end of the time series

% set which cut off we want to use here: tt, hb, lb, mhb
time = hb;


%% Parameterize trials
    %
    V_tR=V(1:proj_tpts(time),:); % Reduced length voltage matrix (to response duration)
...
%% Package data out 
    ...
    %
    crp_projs.tR_index=time; % index into proj_tpts (projection magnitude times) corresponding to response duration
    %
    ...
    crp_projs.t_value_tR=mean(S_all(stat_indices,time))/(std(S_all(stat_indices,time))/sqrt(length(S_all(stat_indices,time)))); % calculate t-statistic

    % p-value at response duration \tau_R (extraction significance)
    [~,crp_projs.p_value_tR]=ttest(S_all(stat_indices,time),0,'tail','right'); % simple t-test
    ...
    crp_parms.tR=t_win(proj_tpts(time)); % response duration in units seconds 
    crp_parms.parms_times=t_win(1:proj_tpts(time)); % times for parameterized data
    % 
